...
def main():
    ...
    # Val分割（train.pyと同じseed・ratioで再現）
    df = pd.read_csv(config["data"]["train_path"])
    df = df[(df["transliteration"].astype(str).str.len() > 0) & (df["translation"].astype(str).str.len() > 0)]
    _, val_df = train_test_split(df, test_size=config["training"]["val_ratio"], random_state=config["training"]["seed"])
    ...
    # === ドキュメントレベル（比較用） ===
    doc_inputs = [prefix + str(t) for t in val_df["transliteration"].astype(str)]
    doc_refs = val_df["translation"].astype(str).tolist()
