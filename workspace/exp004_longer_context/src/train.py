"""
ByT5-small 学習スクリプト (exp004)
変更点:
- encoder_max_length=1024, decoder_max_length=2048 (truncation解消)
- load_best_model_at_end=True (geo_meanベース)
- wandb連携
- MetricsLogger維持（JSON記録）
"""
import os
...
def tokenize_fn(examples, tokenizer, encoder_max_length, decoder_max_length):
    """動的パディング: truncationのみ、paddingはDataCollatorに委任
    入力と出力で異なるmax_lengthを使用"""
...
def main():
    ...
    logger.info(f"Encoder max_length: {config['model']['encoder_max_length']}")
    logger.info(f"Decoder max_length: {config['model']['decoder_max_length']}")
    ...
    train_tokenized = train_dataset.map(
        lambda x: tokenize_fn(x, tokenizer, encoder_max_length, decoder_max_length),
        batched=True,
    ...
    args = Seq2SeqTrainingArguments(
        ...
        generation_max_length=config["model"]["decoder_max_length"],
        logging_steps=50,
        report_to="wandb",
    ...
    # ベストモデル保存（load_best_model_at_end=Trueなので現在のモデルがベスト）
