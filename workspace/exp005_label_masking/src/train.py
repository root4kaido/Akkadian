"""
ByT5-small 学習スクリプト (exp005)
変更点:
- 順方向: encoder=512, decoder=512 + 英語ラベル文末マスキング（preprocess側で実施）
- 逆方向: encoder=1024, decoder=512
...
def tokenize_fn(examples, tokenizer, max_length, reverse_encoder_max_length):
    ...
    """
    # 方向ごとにencoder max_lengthを決定
    input_texts = examples["input_text"]
    target_texts = examples["target_text"]
    directions = examples["direction"]

    # バッチ内で方向別にencoder max_lengthを適用するため、個別にトークナイズ
    all_input_ids = []
    all_attention_masks = []
    all_labels = []
    ...
    for inp, tgt, direction in zip(input_texts, target_texts, directions):
        ...
        dec_max = max_length  # decoder は常に512
