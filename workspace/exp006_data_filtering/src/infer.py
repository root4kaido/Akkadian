...
def main():
    ...
    greedy_kwargs = {"max_length": max_length, "num_beams": num_beams}
    if num_beams > 1:
        greedy_kwargs["early_stopping"] = config["inference"]["early_stopping"]

    raw_preds = run_inference(
        model, tokenizer, val_texts, max_length,
    ...
    test_raw = run_inference(
        model, tokenizer, test_texts, max_length,
