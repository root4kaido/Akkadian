"""
ByT5-base 学習スクリプト (exp009)
変更点 (from exp008):
- 確率的input truncation augmentation（AugmentedTrainDataset使用）
- 各__getitem__でdoc/sent入力を確率的に切り替え
- Val/compute_metricsはexp008と同一
"""
import os
...
def tokenize_fn(examples, tokenizer, max_length, reverse_encoder_max_length):
    """Val用: 方向別のmax_lengthでトークナイズ（exp008と同一）"""
...
def main():
    ...
    logger.info(f"Augmentation: enabled={aug_config.get('enabled')}, prob={aug_config.get('prob')}, max_bytes={aug_config.get('max_bytes')}")

    # データ準備
    os.chdir(EXP_DIR)
    sys.path.insert(0, SCRIPT_DIR)
    from preprocess import prepare_data, AugmentedTrainDataset
    ...
    # Train: カスタムDataset（動的augmentation）
    train_dataset = AugmentedTrainDataset(train_df, config, tokenizer)
    logger.info(f"Train dataset size: {len(train_dataset)}")

    # Val: 静的データセット（exp008と同じ）
    prefix_fwd = config["model"]["prefix"]
    val_prepared = {
        "input_text": [prefix_fwd + str(row["transliteration"]) for _, row in val_df.iterrows()],
        "target_text": [str(row["translation"]) for _, row in val_df.iterrows()],
        "direction": ["forward"] * len(val_df),
    }
    val_dataset_raw = Dataset.from_dict(val_prepared)
    val_tokenized = val_dataset_raw.map(
