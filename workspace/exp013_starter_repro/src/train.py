"""
exp013_starter_repro: Starterノートブック完全再現
- takamichitoda/dpc-starter-train と完全同一の学習ロジック
- インフラのみ改善: wandb, best/last保存, generation_max_length修正
- LB=26の再現を確認し、今後のアブレーションのベースラインとする
"""
import os
import gc
import re
...
def compute_metrics(eval_preds):
    ...
    # starterと同一: chrFはreferences直接、BLEUはリスト化
...
args = Seq2SeqTrainingArguments(
    ...
    per_device_train_batch_size=2,  # RTX4090余裕あり (starter=1)
    per_device_eval_batch_size=2,
    gradient_accumulation_steps=4,  # effective batch = 8 (starterと同一)
    generation_max_length=MAX_LENGTH,  # ★starterのバグ修正（学習には影響なし）
    weight_decay=0.01,
    save_total_limit=2,
    num_train_epochs=EPOCHS,
    predict_with_generate=True,
    logging_steps=10,
    report_to="wandb",  # ★インフラ改善: wandb追加
    run_name="exp013_starter_repro",
    load_best_model_at_end=True,  # ★インフラ改善: ベストモデル自動選択
...
# load_best_model_at_end=Trueなので、現在のモデルはbest。
# lastモデルは最後のcheckpointから取得する。
last_ckpt = None
