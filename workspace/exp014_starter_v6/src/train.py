"""
exp014_starter_v6: Starterノートブック v6(ピン留め版)の再現
- v6の設定: lr=2e-4, label_smoothing=0, batch=4×accum=2
- latestとの差分: lr(2e-4 vs 1e-4), label_smoothing(0 vs 0.2), batch(4×2 vs 1×8)
- インフラのみ改善: wandb, best/last保存, generation_max_length修正
"""
import os
import gc
import re
...
# transformersのloggerにもFileHandlerを追加
...
# ============================================================
# Config (starter v6 ピン留め版と同一)
# ============================================================
MODEL_NAME = "google/byt5-small"
MAX_LENGTH = 512
EPOCHS = 20
LEARNING_RATE = 2e-4  # v6: 2e-4 (latestは1e-4)
SEED = 42
...
args = Seq2SeqTrainingArguments(
    ...
    optim="adafactor",
    # v6にはlabel_smoothingなし
    fp16=False,  # FP32 (starterと同一)
    per_device_train_batch_size=4,   # v6: 4 (latestは1)
    per_device_eval_batch_size=4,
    gradient_accumulation_steps=2,   # v6: 2 (latestは8) → effective batch = 8
...
logger.info("Starting Training (FP32 mode, starter v6 reproduction)...")
if resume_ckpt:
...
# ============================================================
# Save best model (load_best_model_at_endで自動ロード済み)
# ============================================================
...
# ============================================================
# Save last model (最終epochのモデル = starterが使っていたもの)
# ============================================================
