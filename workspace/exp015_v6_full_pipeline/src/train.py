"""
exp015_v6_full_pipeline: exp011(ByT5-base + PN/GN + 追加データ) + v6設定(lr=2e-4, LS=0)
config.yamlから全パラメータを読み込む。
"""
import os
...
def main():
    ...
    logger.info(f"LR: {config['training']['learning_rate']}, LS: {config['training']['label_smoothing']}")
    ...
    # ラストモデル保存
    last_dir = os.path.join(EXP_DIR, "results", "last_model")
    last_ckpt = None
    ...
    if last_ckpt and os.path.isdir(last_ckpt):
        ...
        last_model = AutoModelForSeq2SeqLM.from_pretrained(last_ckpt)
