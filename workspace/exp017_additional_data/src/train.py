"""
exp017_additional_data: exp014(v6設定) + 追加データのみ
- exp014との差分: 追加データ(Sentences_Oare + published_texts)をtrain splitに追加
- ByT5-small、タグなし、その他全てexp014と同一
- Valはtrain.csvのみから分割（追加データ投入前に分割、公平な評価のため）
"""
import os
import gc
import re
...
# Config — exp014と同一
# ============================================================
MODEL_NAME = "google/byt5-small"  # exp014と同一
MAX_LENGTH = 512
EPOCHS = 20
LEARNING_RATE = 2e-4
SEED = 42
...
# exp014と同一のsplit（HuggingFace datasets）
dataset = Dataset.from_pandas(train_expanded)
split_datasets = dataset.train_test_split(test_size=0.1, seed=42)

# 追加データをtrain splitにのみ追加
additional_path = EXP_DIR / "dataset" / "additional_train.csv"
if additional_path.exists():
    additional_df = pd.read_csv(str(additional_path))
    ...
    logger.info(f"Additional data: {len(additional_df)} rows")

    # train splitにのみ追加（valは汚染しない）
    train_pandas = split_datasets["train"].to_pandas()
    train_pandas = pd.concat([train_pandas, additional_df], ignore_index=True)
    split_datasets["train"] = Dataset.from_pandas(train_pandas)
    logger.info(f"Train after adding additional data: {len(split_datasets['train'])} rows")
else:
    logger.warning(f"Additional data not found: {additional_path}")
...
args = Seq2SeqTrainingArguments(
    ...
    logging_steps=10,
    report_to="wandb",
    run_name="exp017_additional_data",
...
logger.info("Starting Training (FP32, ByT5-small + additional data, v6 settings)...")
if resume_ckpt:
