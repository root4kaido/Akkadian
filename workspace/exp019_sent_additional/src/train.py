"""
exp019_sent_additional: 元doc-levelデータ + 文レベル分割データ(2文目以降)を追加
- ベース: exp016 (ByT5-base, bf16, lr=2e-4, 20epoch)
- データ: train.csv (doc-level) + sentence_aligned.csv (sent_idx >= 1 のみ追加)
- 1文目は元docのtruncationに含まれるため重複回避で除外
"""
import os
import gc
import re
...
# ============================================================
# Train data: 元doc-level + sentence_aligned.csv (2文目以降)
# ============================================================
...
# val内のtransliterationを集めてフィルタ（oare_idがないのでtransliterationで判定）
# ただしval splitはdoc-levelなので、sentence_aligned側のakk_origと比較
val_translit_set = set(val_data['transliteration'].astype(str).tolist())
sent_additional_filtered = sent_additional[
    ~sent_additional['akk_orig'].astype(str).isin(val_translit_set)
].copy()
...
args = Seq2SeqTrainingArguments(
    ...
    logging_steps=10,
    report_to="wandb",
    run_name="exp019_sent_additional",
...
logger.info("Starting Training (exp019: sent_additional)...")
if resume_ckpt:
