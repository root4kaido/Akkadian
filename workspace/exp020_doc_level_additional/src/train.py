"""
exp020_doc_level_additional: 元doc-levelデータ + 開始位置をずらしたdoc-levelデータを追加
- ベース: exp016 (ByT5-base, bf16, lr=2e-4, 20epoch)
- データ: train.csv (doc-level) + sentence_aligned.csvの2文目以降をdoc-levelで追加
  - 3文docの場合: (sent2+sent3)→(eng2+eng3), (sent3)→(eng3) の2件追加
  - exp019との違い: 個別文ペアではなくdoc-levelペアとして追加
"""
import os
import gc
import re
...
# ============================================================
# Train data: 元doc-level + sentence_aligned 2文目以降をdoc-levelで追加
# ============================================================
...
# 各oare_idについて、開始位置をずらしたdoc-levelペアを作成
...
for oare_id, group in sent_aligned_filtered.groupby('oare_id'):
    ...
    # start=1から始めて、各位置から末尾までを結合
    for start in range(1, n):
        akk_doc = " ".join(akk_segments[start:])
        eng_doc = " ".join(eng_sentences[start:])
        if akk_doc.strip() and eng_doc.strip():
            additional_rows_list.append({
                "transliteration": akk_doc,
                "translation": eng_doc,
            })

additional_rows = pd.DataFrame(additional_rows_list)
logger.info(f"Doc-level additional data: {len(additional_rows)} pairs (shifted start positions)")
...
args = Seq2SeqTrainingArguments(
    ...
    logging_steps=10,
    report_to="wandb",
    run_name="exp020_doc_level_additional",
...
logger.info("Starting Training (exp020: doc_level_additional)...")
if resume_ckpt:
