"""
exp023: Kaggle提出用推論 v5 (Round-trip rerank, rt_weighted)
- 3温度(0.2/0.4/0.6) × 1候補 = 3候補
- 各候補を逆翻訳 (Eng→Akk) し、元ソースとのweightedスコアが最大の候補を選択
  chrF++(0.55) + BLEU(0.25) + Jaccard(0.20) + LenBonus(0.10)
- 動的パディング + 長さソート
...
if ON_KAGGLE:
    ...
    OUTPUT_PATH = os.path.join(EXP_DIR, "results", "submission_v5_rt_weighted.csv")
...
# ============================================================
# Round-trip scoring: weighted (chrF++ 0.55 + BLEU 0.25 + Jaccard 0.20 + LenBonus 0.10)
# ============================================================
...
# Step 3: Round-trip rerank (weighted of BT vs source)
print("=== Step 3: Round-trip rerank (rt_weighted) ===")
all_predictions = []
for i in tqdm(range(n_samples), desc="rerank"):
    ...
    src = source_texts[i]
    best = rt_pick_weighted(cands, bts, src)
