...
if ON_KAGGLE:
    ...
    OUTPUT_PATH = os.path.join(EXP_DIR, "results", "submission_v6_ensemble_rt.csv")
...
# ============================================================
# Step 1: Model A — forward translate + back-translate
# ============================================================
print("=" * 60)
...
# Back-translation (greedy)
rev_texts_a = [PREFIX_REV + p for p in preds_a]
rev_dataset_a = InferenceDataset(rev_texts_a, tokenizer_a)
bt_a = generate_sorted(rev_dataset_a, tokenizer_a, model_a, n_samples, desc="A BT", num_beams=4)
print(f"  A bt[0]: {bt_a[0][:80]}...")

# Save to file
cache_a = {"predictions": preds_a, "back_translations": bt_a}
...
# Release memory
del model_a, tokenizer_a, fwd_dataset_a, rev_dataset_a
torch.cuda.empty_cache()
print("Model A released from GPU.")

# ============================================================
# Step 2: Model B — forward translate + back-translate
# ============================================================
print("=" * 60)
...
# Back-translation (greedy)
rev_texts_b = [PREFIX_REV + p for p in preds_b]
rev_dataset_b = InferenceDataset(rev_texts_b, tokenizer_b)
bt_b = generate_sorted(rev_dataset_b, tokenizer_b, model_b, n_samples, desc="B BT", num_beams=4)
print(f"  B bt[0]: {bt_b[0][:80]}...")

# Save to file
cache_b = {"predictions": preds_b, "back_translations": bt_b}
...
# Release memory
del model_b, tokenizer_b, fwd_dataset_b, rev_dataset_b
torch.cuda.empty_cache()
print("Model B released from GPU.")

# ============================================================
# Step 3: Round-trip weighted rerank (2 candidates)
# ============================================================
print("=" * 60)
print("=== Step 3: Round-trip weighted rerank ===")
print("=" * 60)
...
bt_a = cache_a["back_translations"]
preds_b = cache_b["predictions"]
bt_b = cache_b["back_translations"]
...
for i in tqdm(range(n_samples), desc="rerank"):
    ...
    back_translations = [bt_a[i], bt_b[i]]
    src = source_texts[i]
