"""
exp024_pn_gn_tagging: exp023(全前処理) + OA_LexiconベースPN/GNタグ付加
変更点（exp023から）:
  - transliterationのトークンにOA_Lexiconの[PN]/[GN]タグを付加
  - 固有名詞認識の強化（val全体で25.5%の固有名詞ミスの改善を期待）
  - それ以外はexp023と完全同一
"""
import os
import gc
...
# ============================================================
# PN/GN tagging: OA_Lexiconの辞書でトークンにタグ付加
# ============================================================
...
n_tagged = train_df["transliteration"].str.contains(r'\[(?:PN|GN)\]', regex=True).sum()
logger.info(f"PN/GN tagging: {n_tagged}/{len(train_df)} docs contain tags")
