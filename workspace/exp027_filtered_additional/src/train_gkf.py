"""
exp027_filtered_additional: GroupKFold版
- exp023(前処理あり) + フィルタ済み追加データ(additional_train_filtered.csv)
...
ADDITIONAL_DATA_PATH = PROJECT_ROOT / "datasets" / "processed" / "additional_train_filtered.csv"
...
# ============================================================
# 追加データ: additional_train_filtered.csv
# ============================================================
additional_df = pd.read_csv(str(ADDITIONAL_DATA_PATH))
logger.info(f"Filtered additional data: {len(additional_df)} rows")

# 前処理適用（追加データにも同じ前処理）
additional_df["translation"] = additional_df["translation"].astype(str).apply(clean_translation)
additional_df["transliteration"] = additional_df["transliteration"].astype(str).apply(clean_transliteration)
logger.info(f"Additional data after preprocessing: {len(additional_df)} rows")

# 結合（追加データは外部ソースのためAKTグループ除外不要）
train_combined = pd.concat([
    train_split[["transliteration", "translation"]],
    additional_df[["transliteration", "translation"]]
], ignore_index=True)
logger.info(f"Combined train data: {len(train_combined)} samples (+{len(additional_df)} additional)")
...
args = Seq2SeqTrainingArguments(
    ...
    logging_steps=10,
    report_to="wandb",
    run_name=f"exp027_gkf_fold{FOLD}",
...
logger.info(f"Starting Training (exp027 GKF fold{FOLD})...")
if resume_ckpt:
