"""
exp028_sent_finetune: Stage2 sentence-level fine-tuning
- exp023 fold3 last_modelをロードし、sentence_aligned.csvで追加fine-tune
- E層(geo 10-30, 840件36.3%)の底上げを狙う
- sentence_aligned.csvのval oare_idを除外
- 低lr (5e-5), 少epochs (5)
"""
import os
import gc
...
# Stage1 model: exp023 fold3 last_model
STAGE1_MODEL_DIR = str(PROJECT_ROOT / "workspace" / "exp023_full_preprocessing" / "results" / f"fold{FOLD}" / "last_model")
...
logger.info(f"Stage1 model: {STAGE1_MODEL_DIR}")
...
# ============================================================
# Data: GroupKFoldでval splitを再現 → val oare_idsを特定
# ============================================================
...
# GroupKFold splitを再現してval oare_idsを取得
...
logger.info(f"Fold {FOLD}: val={len(val_split)}, val_oare_ids={len(val_oare_ids)}")
logger.info(f"Val groups:\n{val_groups.to_string()}")

# ============================================================
# Stage2 学習データ: sentence_aligned.csv (sent_idx >= 1) からval除外
# ============================================================
sent_aligned = pd.read_csv(str(SENTENCE_ALIGNED_PATH))
sent_data = sent_aligned.copy()
logger.info(f"Sentence aligned data (all): {len(sent_data)} sentences from {sent_data['oare_id'].nunique()} docs")

# val oare_idsを除外
sent_train = sent_data[~sent_data['oare_id'].isin(val_oare_ids)].copy()
logger.info(f"After excluding val docs: {len(sent_train)} sentences from {sent_train['oare_id'].nunique()} docs")

# 前処理適用
sent_train_df = pd.DataFrame({
    "transliteration": sent_train['akk_segment'].astype(str).apply(clean_transliteration),
    "translation": sent_train['eng_sentence'].astype(str).apply(clean_translation),
})

# バイト長統計
sent_train_df["src_bytes"] = sent_train_df["transliteration"].str.len()
logger.info(f"Sentence train data byte distribution:")
for lo, hi in [(0, 50), (50, 100), (100, 200), (200, 500)]:
    sub = sent_train_df[(sent_train_df["src_bytes"] >= lo) & (sent_train_df["src_bytes"] < hi)]
    logger.info(f"  {lo:4d}-{hi:4d} bytes: {len(sub):4d} ({len(sub)/len(sent_train_df)*100:.1f}%)")
sent_train_df = sent_train_df.drop(columns=["src_bytes"])

logger.info(f"Stage2 train samples: {len(sent_train_df)}")
...
bidirectional_train = create_bidirectional_data_from_df(sent_train_df)
...
# ============================================================
# Training: Stage1 modelをロードしてfine-tune
# ============================================================
gc.collect()
torch.cuda.empty_cache()

logger.info(f"Loading Stage1 model from {STAGE1_MODEL_DIR}")
model = AutoModelForSeq2SeqLM.from_pretrained(STAGE1_MODEL_DIR)
...
args = Seq2SeqTrainingArguments(
    ...
    logging_steps=10,
    report_to="wandb",
    run_name=f"exp028_stage2_fold{FOLD}",
...
logger.info(f"Starting Stage2 Training (exp028 fold{FOLD})...")
if resume_ckpt:
...
logger.info(f"Stage2 Training complete. Fold {FOLD}")
