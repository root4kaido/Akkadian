"""
exp031_dapt: DAPT済みモデルでfine-tune（GroupKFold版）
- DAPT事前学習済みByT5-baseからfine-tune
- それ以外はexp023 train_gkf.pyと同一
"""
import os
import gc
import re
...
# Config
# ============================================================
# DAPT pre-trained model (best by eval_loss)
DAPT_MODEL_DIR = EXP_DIR / "results" / "dapt_model"
MODEL_NAME = str(DAPT_MODEL_DIR)
MAX_LENGTH = 512
EPOCHS = 20
LEARNING_RATE = 2e-4
SEED = 42
...
args = Seq2SeqTrainingArguments(
    ...
    logging_steps=10,
    report_to="wandb",
    run_name=f"exp031_dapt_finetune_fold{FOLD}",
...
logger.info(f"Starting Training (exp031 DAPT finetune fold{FOLD})...")
logger.info(f"DAPT model: {MODEL_NAME}")
if resume_ckpt:
