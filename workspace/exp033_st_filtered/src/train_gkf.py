"""
exp032_self_training (filtered): Self-Training版GroupKFold
- pseudo dataを品質フィルタ + real dataと1:1比率にダウンサンプリング
- 短文除外（translation < 20文字）
"""
import os
import gc
import re
...
# Pseudo labels読み込み
PSEUDO_LABELS_PATH = PROJECT_ROOT / "workspace" / "exp032_self_training" / "results" / "pseudo_labels.csv"
pseudo_df = pd.read_csv(str(PSEUDO_LABELS_PATH))
logger.info(f"Pseudo labels loaded: {len(pseudo_df)} docs")
# transliterationは生成時に前処理済み、translationも後処理済み
...
# Pseudo dataの文分割
pseudo_expanded = simple_sentence_aligner(pseudo_df[["oare_id", "transliteration", "translation"]])
logger.info(f"Expanded Pseudo Data (raw): {len(pseudo_expanded)} sentences")

# 品質フィルタ: 短文除外
pseudo_expanded = pseudo_expanded[pseudo_expanded["translation"].str.len() >= 20].reset_index(drop=True)
logger.info(f"After quality filter (>=20 chars): {len(pseudo_expanded)} sentences")

# oare_id → akt_group マッピング（pseudo行は"pseudo"グループ）
train_expanded["akt_group"] = train_expanded["oare_id"].map(oare_to_group).fillna("None")
pseudo_expanded["akt_group"] = "pseudo"

# GroupKFold split（real dataのみでsplit、pseudoは常にtrain側）
groups = train_expanded["akt_group"].values
gkf = GroupKFold(n_splits=5)
splits = list(gkf.split(train_expanded, groups=groups))
train_idx, val_idx = splits[FOLD]

train_split_real = train_expanded.iloc[train_idx].copy()
val_split = train_expanded.iloc[val_idx].copy()

# Pseudo を real と同数にダウンサンプリング（1:1比率）
n_real = len(train_split_real)
if len(pseudo_expanded) > n_real:
    pseudo_sampled = pseudo_expanded.sample(n=n_real, random_state=SEED).reset_index(drop=True)
    logger.info(f"Downsampled pseudo: {len(pseudo_expanded)} -> {len(pseudo_sampled)}")
else:
    pseudo_sampled = pseudo_expanded

# Real train + Pseudo を結合
train_split = pd.concat([train_split_real, pseudo_sampled], ignore_index=True)
logger.info(f"Combined train: {len(train_split)} (real={len(train_split_real)}, pseudo={len(pseudo_sampled)})")
...
args = Seq2SeqTrainingArguments(
    ...
    logging_steps=10,
    report_to="wandb",
    run_name=f"exp033_st_filtered_fold{FOLD}",
...
logger.info(f"Starting Training (exp033 self-training filtered fold{FOLD})...")
if resume_ckpt:
