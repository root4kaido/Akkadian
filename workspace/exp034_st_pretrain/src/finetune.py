"""
exp034_st_pretrain: Stage2 — pretrain済みモデルでreal dataをfine-tune
- Stage1のpretrain_modelをロード
- real dataで20epoch fine-tune（exp023と同じ設定）
- 保存先: results/fold{N}/best_model, last_model
"""
import os
import gc
import re
...
BEST_LOSS_MODEL_DIR = str(RESULTS_DIR / "best_loss_model")
...
args = Seq2SeqTrainingArguments(
    ...
    logging_steps=10,
    report_to="wandb",
    run_name=f"exp034_finetune_fold{FOLD}",
...
logger.info(f"Starting Stage2 Fine-tune (exp034 fold{FOLD}, {EPOCHS} epochs on real data)...")
if resume_ckpt:
...
# Save models
# ============================================================
logger.info(f"Saving best_loss model to {BEST_LOSS_MODEL_DIR}")
trainer.save_model(BEST_LOSS_MODEL_DIR)
tokenizer.save_pretrained(BEST_LOSS_MODEL_DIR)
...
logger.info(f"Best loss model eval metrics: {metrics}")
...
logger.info(f"Best loss model: {BEST_LOSS_MODEL_DIR}")
