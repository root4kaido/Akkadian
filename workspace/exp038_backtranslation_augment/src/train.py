"""
exp038_backtranslation_augment: exp023ベース + 逆翻訳データ混合学習
- generated_english.csv → s1_exp007(byt5-large)で逆翻訳したデータを追加
- AKTグループでGroupKFold(n_splits=5)
- --fold で実行するfoldを指定
- byt5-base, 20 epochs, bidirectional training
"""
import os
import gc
import re
...
# ============================================================
# Back-translated data の追加
# ============================================================
if not BT_DATA_PATH.exists():
    logger.error(f"Back-translated data not found: {BT_DATA_PATH}")
    logger.error("Run generate_backtranslation.py first!")
    sys.exit(1)

bt_df = pd.read_csv(str(BT_DATA_PATH))
logger.info(f"Back-translated data: {len(bt_df)} samples")

# BTデータを train_split に追加（valには入れない）
# Note: BTデータのtransliterationはs1_exp007(clean_transliteration適用済みモデル)の出力なので再適用不要
bt_train = bt_df[["transliteration", "translation"]].copy()
real_train = train_split[["transliteration", "translation"]].copy()

combined_train = pd.concat([real_train, bt_train], ignore_index=True)
logger.info(f"Combined train: {len(combined_train)} (real={len(real_train)}, bt={len(bt_train)})")
logger.info(f"Ratio bt/real: {len(bt_train)/len(real_train):.2f}")
...
logger.info(f"Train samples: {len(bidirectional_train)} (Bidirectional, real+bt)")
...
args = Seq2SeqTrainingArguments(
    ...
    logging_steps=10,
    report_to="wandb",
    run_name=f"exp038_bt_augment_fold{FOLD}",
...
logger.info(f"Starting Training (exp038 BT augment fold{FOLD})...")
if resume_ckpt:
