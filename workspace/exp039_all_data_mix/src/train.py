"""
exp039_all_data_mix: 全データ混合学習
- train.csv + additional_train_filtered + pseudo_labels_v2 + backtranslated
- AKTグループでGroupKFold(n_splits=5) — valはtrain.csvのみ
- --fold で実行するfoldを指定
- byt5-base, 5 epochs, bidirectional training
"""
import os
import gc
import re
...
ADDITIONAL_TRAIN_PATH = PROJECT_ROOT / "datasets" / "processed" / "additional_train_filtered.csv"
PSEUDO_V2_PATH = PROJECT_ROOT / "workspace" / "exp038_backtranslation_augment" / "dataset" / "pseudo_labels_v2.csv"
BT_DATA_PATH = PROJECT_ROOT / "workspace" / "exp038_backtranslation_augment" / "dataset" / "backtranslated.csv"
...
# 1. additional_train_filtered
add_df = pd.read_csv(str(ADDITIONAL_TRAIN_PATH))
add_df["transliteration"] = add_df["transliteration"].astype(str).apply(clean_transliteration)
add_df["translation"] = add_df["translation"].astype(str).apply(clean_translation)
extra_dfs.append(("additional_train_filtered", add_df[["transliteration", "translation"]]))
logger.info(f"additional_train_filtered: {len(add_df)} samples")

# 2. pseudo_labels_v2
pseudo_df = pd.read_csv(str(PSEUDO_V2_PATH))
# transliterationは元データ(clean_transliteration適用済み)、translationはs1_exp007出力(clean_translation適用済み)
extra_dfs.append(("pseudo_labels_v2", pseudo_df[["transliteration", "translation"]]))
logger.info(f"pseudo_labels_v2: {len(pseudo_df)} samples")

# 3. backtranslated
bt_df = pd.read_csv(str(BT_DATA_PATH))
# transliterationはs1_exp007出力、translationはgenerated_english(clean_translation適用済み)
extra_dfs.append(("backtranslated", bt_df[["transliteration", "translation"]]))
logger.info(f"backtranslated: {len(bt_df)} samples")

# ============================================================
# 全データ結合
# ============================================================
real_train = train_split[["transliteration", "translation"]].copy()
all_extra = pd.concat([df for _, df in extra_dfs], ignore_index=True)

combined_train = pd.concat([real_train, all_extra], ignore_index=True)
logger.info(f"Combined train: {len(combined_train)} (real={len(real_train)}, extra={len(all_extra)})")
for name, df in extra_dfs:
    logger.info(f"  {name}: {len(df)}")
logger.info(f"Ratio extra/real: {len(all_extra)/len(real_train):.2f}")
...
bidirectional_train = create_bidirectional_data_from_df(combined_train)
unidirectional_val = create_unidirectional_data_from_df(val_split)
logger.info(f"Train samples: {len(bidirectional_train)} (Bidirectional, all data)")
logger.info(f"Val samples:   {len(unidirectional_val)} (Unidirectional, real only)")
...
args = Seq2SeqTrainingArguments(
    ...
    logging_steps=10,
    report_to="wandb",
    run_name=f"exp039_all_data_mix_fold{FOLD}",
...
logger.info(f"Starting Training (exp039 all data mix fold{FOLD})...")
if resume_ckpt:
