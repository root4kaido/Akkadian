"""
exp045_bt_large: 2段階学習 (exp044のlarge版)
Stage 1: BT(1/4サンプリング ~15k) + pseudo_labels_v2(6k) で pretrain 5ep
Stage 2: real train で finetune 5ep
- byt5-large, bidirectional training
...
parser.add_argument("--stage", type=int, default=0, help="0=auto, 1=pretrain only, 2=finetune only")
cmd_args = parser.parse_args()
FOLD = cmd_args.fold
STAGE_OVERRIDE = cmd_args.stage
...
RESULTS_DIR = EXP_DIR / "results" / f"fold{FOLD}" / "pretrain_ft"
RESULTS_DIR.mkdir(parents=True, exist_ok=True)

AKT_GROUPS_PATH = PROJECT_ROOT / "datasets" / "processed" / "akt_groups.csv"
BT_DATA_PATH = PROJECT_ROOT / "workspace" / "exp044_bt_augment_v3" / "dataset" / "backtranslated.csv"
PSEUDO_DATA_PATH = PROJECT_ROOT / "workspace" / "exp038_backtranslation_augment" / "dataset" / "pseudo_labels_v2.csv"
...
PRETRAIN_CKPT_DIR = str(RESULTS_DIR / "checkpoints_pretrain")
FINETUNE_CKPT_DIR = str(RESULTS_DIR / "checkpoints_finetune")

device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
logger.info(f"Device: {device}")
logger.info(f"Fold: {FOLD}, Stage override: {STAGE_OVERRIDE}")
...
logger.info(f"Expanded Train Data (real): {len(train_expanded)} sentences")
...
logger.info(f"Fold {FOLD}: real_train={len(train_split)}, val={len(val_split)}")

# ============================================================
# Load pseudo data (BT + published_texts pseudo labels)
# ============================================================
bt_df = pd.read_csv(str(BT_DATA_PATH))
logger.info(f"Back-translated data (full): {len(bt_df)} samples")

bt_df = bt_df.sample(frac=BT_SAMPLE_FRAC, random_state=SEED).reset_index(drop=True)
logger.info(f"Back-translated data (1/4 sampled): {len(bt_df)} samples")

pseudo_df = pd.read_csv(str(PSEUDO_DATA_PATH))
logger.info(f"Pseudo labels v2: {len(pseudo_df)} samples")

pseudo_all = pd.concat([
    bt_df[["transliteration", "translation"]],
    pseudo_df[["transliteration", "translation"]],
], ignore_index=True)
logger.info(f"Combined pseudo data: {len(pseudo_all)} (bt={len(bt_df)}, pseudo={len(pseudo_df)})")
...
# Datasets
pretrain_dataset = create_bidirectional_data_from_df(pseudo_all)
finetune_dataset = create_bidirectional_data_from_df(train_split)
val_dataset = create_unidirectional_data_from_df(val_split)

logger.info(f"Pretrain samples: {len(pretrain_dataset)} (bt+pseudo, bidirectional)")
logger.info(f"Finetune samples: {len(finetune_dataset)} (real, bidirectional)")
logger.info(f"Val samples: {len(val_dataset)} (real, unidirectional)")
...
tokenized_pretrain = pretrain_dataset.map(preprocess_function, batched=True)
...
# ============================================================
# Stage 1: Pretrain on pseudo data
# ============================================================
should_pretrain = STAGE_OVERRIDE in (0, 1)
should_finetune = STAGE_OVERRIDE in (0, 2)

if should_pretrain:
    pretrain_done_marker = RESULTS_DIR / "pretrain_done"
    if pretrain_done_marker.exists() and STAGE_OVERRIDE == 0:
        logger.info("Pretrain already completed (marker found). Skipping to finetune.")
        should_pretrain = False

if should_pretrain:
    logger.info("=" * 60)
    logger.info("=== Stage 1: Pretrain on BT + pseudo data ===")
    logger.info("=" * 60)
    ...
    pretrain_args = Seq2SeqTrainingArguments(
        ...
        logging_steps=10,
        report_to="wandb",
        run_name=f"exp045_pretrain_ft_stage1_fold{FOLD}",
        load_best_model_at_end=False,
        seed=SEED,
        dataloader_num_workers=12,
    )

    metrics_logger_pt = MetricsLogger(str(RESULTS_DIR / "metrics_log_pretrain.json"))
    ...
    trainer_pt = Seq2SeqTrainer(
        ...
        callbacks=[metrics_logger_pt],
    )

    # チェックポイント自動再開
    ckpt_dir = Path(PRETRAIN_CKPT_DIR)
    resume_ckpt = None
    if ckpt_dir.exists():
        ...
        if checkpoints:
            ...
            logger.info(f"Auto-resuming pretrain from: {resume_ckpt}")

    logger.info("Starting pretrain...")
    if resume_ckpt:
        trainer_pt.train(resume_from_checkpoint=resume_ckpt)
    else:
        trainer_pt.train()

    # Save pretrain model
    logger.info(f"Saving pretrain model to {PRETRAIN_MODEL_DIR}")
    trainer_pt.save_model(PRETRAIN_MODEL_DIR)
    tokenizer.save_pretrained(PRETRAIN_MODEL_DIR)

    pretrain_metrics = trainer_pt.evaluate()
    logger.info(f"Pretrain eval: {pretrain_metrics}")

    # Mark pretrain as done
    with open(str(RESULTS_DIR / "pretrain_done"), "w") as f:
        f.write("done")

    del trainer_pt, model
    gc.collect()
    torch.cuda.empty_cache()
...
if should_finetune:
    logger.info("=" * 60)
    logger.info("=== Stage 2: Finetune on real data ===")
    logger.info("=" * 60)

    if not Path(PRETRAIN_MODEL_DIR).exists():
    ...
    gc.collect()
    torch.cuda.empty_cache()
    model = AutoModelForSeq2SeqLM.from_pretrained(PRETRAIN_MODEL_DIR)
    data_collator = DataCollatorForSeq2Seq(tokenizer, model=model)
    ...
    finetune_args = Seq2SeqTrainingArguments(
        ...
        logging_steps=10,
        report_to="wandb",
        run_name=f"exp045_pretrain_ft_stage2_fold{FOLD}",
    ...
    metrics_logger_ft = MetricsLogger(str(RESULTS_DIR / "metrics_log_finetune.json"))
    ...
    if ckpt_dir.exists():
        ...
        if checkpoints:
            latest = sorted(checkpoints, key=lambda x: int(x.name.split("-")[1]))[-1]
            resume_ckpt = str(latest)
            logger.info(f"Auto-resuming finetune from: {resume_ckpt}")

    logger.info("Starting finetune...")
    if resume_ckpt:
        trainer_ft.train(resume_from_checkpoint=resume_ckpt)
    else:
        trainer_ft.train()
    ...
    ckpts = sorted(Path(FINETUNE_CKPT_DIR).glob("checkpoint-*"), key=lambda p: int(p.name.split("-")[1]))
    if ckpts:
        last_model = AutoModelForSeq2SeqLM.from_pretrained(str(ckpts[-1]))
        last_model.save_pretrained(LAST_MODEL_DIR)
        tokenizer.save_pretrained(LAST_MODEL_DIR)
        del last_model
    else:
        trainer_ft.save_model(LAST_MODEL_DIR)
        tokenizer.save_pretrained(LAST_MODEL_DIR)

    final_metrics = trainer_ft.evaluate()
