"""
s1_exp004_large_lora: byt5-large + LoRA fine-tuning
- AKTグループでGroupKFold(n_splits=5)
- --fold で実行するfoldを指定
- LoRA (r=16, alpha=32) でパラメータ効率的にfine-tune
"""
import os
import gc
import re
...
args = Seq2SeqTrainingArguments(
    ...
    logging_steps=10,
    report_to="wandb",
    run_name=f"s1_exp004_large_lora_fold{FOLD}",
...
# Save models
# ============================================================
# LoRA: merge weights and save as full model
logger.info(f"Merging LoRA weights and saving best model to {BEST_MODEL_DIR}")
merged_model = model.merge_and_unload()
merged_model.save_pretrained(BEST_MODEL_DIR)
tokenizer.save_pretrained(BEST_MODEL_DIR)

# For LoRA, last_model = best_model (load_best_model_at_end=True)
