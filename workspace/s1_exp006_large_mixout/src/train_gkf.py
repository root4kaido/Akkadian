"""
s1_exp006_large_mixout: byt5-large + Mixout正則化
- AKTグループでGroupKFold(n_splits=5)
- --fold で実行するfoldを指定
- Mixout(p=0.5): 学習中にランダムにpretrained weightsへリセット
"""
import os
import gc
import re
...
class MixoutLinear(nn.Module):
    """Mixout: dropout that resets to pretrained weights instead of zero."""
    def __init__(self, pretrained_weight, pretrained_bias, in_features, out_features, mixout_prob=0.5):
        super().__init__()
        self.in_features = in_features
        self.out_features = out_features
        self.mixout_prob = mixout_prob
        self.register_buffer("pretrained_weight", pretrained_weight.clone())
        if pretrained_bias is not None:
            self.register_buffer("pretrained_bias", pretrained_bias.clone())
        else:
            self.register_buffer("pretrained_bias", None)
    ...
    def forward(self, x):
        if self.training:
            mask = torch.bernoulli(torch.full_like(self.weight, self.mixout_prob)).bool()
            mixed_weight = torch.where(mask, self.pretrained_weight, self.weight)
            mixed_bias = None
            if self.bias is not None and self.pretrained_bias is not None:
                bias_mask = torch.bernoulli(torch.full_like(self.bias, self.mixout_prob)).bool()
                mixed_bias = torch.where(bias_mask, self.pretrained_bias, self.bias)
            return nn.functional.linear(x, mixed_weight, mixed_bias)
        else:
            return nn.functional.linear(x, self.weight, self.bias)


def apply_mixout(model, mixout_prob=0.5):
    """Replace Linear layers with MixoutLinear."""
    count = 0
    for name, module in model.named_modules():
        if isinstance(module, nn.Linear):
            ...
            new_module = MixoutLinear(
                module.weight.data, module.bias.data if module.bias is not None else None,
                module.in_features, module.out_features, mixout_prob,
            )
...
# Apply Mixout to all Linear layers
MIXOUT_PROB = 0.5
n_replaced = apply_mixout(model, mixout_prob=MIXOUT_PROB)
logger.info(f"Applied Mixout(p={MIXOUT_PROB}) to {n_replaced} Linear layers")
...
args = Seq2SeqTrainingArguments(
    ...
    logging_steps=10,
    report_to="wandb",
    run_name=f"s1_exp006_large_mixout_fold{FOLD}",
...
logger.info(f"Starting Training (s1_exp002 large lowlr GKF fold{FOLD})...")
if resume_ckpt:
