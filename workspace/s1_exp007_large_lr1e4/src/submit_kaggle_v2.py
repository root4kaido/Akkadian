"""
s1_exp007_large_lr1e4: Kaggle提出用推論 v2
- 動的パディング + 長さソートで高速化（sent-CVで1.62x実証済み）
- beam4, max_length=512, early_stopping=True
- repeat_cleanup後処理
- テスト入力: clean_transliteration() + (ki)→{ki} + 近似小数マッチ
...
if ON_KAGGLE:
    MODEL_BASE = "/kaggle/input/models/nomorevotch/akkadianmodels/pytorch/s1_exp007_large_lr1e4/1"
...
with torch.no_grad():
    for batch in tqdm(test_loader, desc="Inference"):
        ...
        sorted_predictions.extend([d.strip() for d in decoded])

# 元の順序に復元
all_predictions = [""] * len(test_df)
for new_idx, pred in enumerate(sorted_predictions):
    all_predictions[idx_map[new_idx]] = pred
