"""
s1_exp008_large_st_pretrain: Stage2 — pretrain済みモデルでreal dataをfine-tune
- exp034_st_pretrain/finetune.pyベース
- Stage1のpretrain_model (byt5-large) をロード
- real dataで5epoch fine-tune (lr=2.5e-5)
- _fast_batch_decode (byt5-largeハング対策)
- GatedBestModelCallback (過学習制御)
"""
import os
import gc
import re
...
# Stage1のpretrain model
PRETRAIN_MODEL_DIR = str(RESULTS_DIR / "pretrain_model")

log_file = str(RESULTS_DIR / "finetune.log")
...
class GatedBestModelCallback(TrainerCallback):
    """geo_mean最大 かつ loss < min_loss * threshold の時のみ保存"""
    def __init__(self, save_dir, tokenizer, loss_gate=1.2):
        self.save_dir = save_dir
        self.tokenizer = tokenizer
        self.loss_gate = loss_gate
        self.best_loss = float("inf")
        self.best_geo = -float("inf")
    ...
    def on_evaluate(self, args, state, control, metrics=None, model=None, **kwargs):
        ...
        if loss < self.best_loss:
            self.best_loss = loss
        if geo > self.best_geo and loss <= self.best_loss * self.loss_gate:
            self.best_geo = geo
            model.save_pretrained(self.save_dir)
            self.tokenizer.save_pretrained(self.save_dir)
            logger.info(f"Gated best saved: epoch={metrics.get('epoch')}, geo={geo}, loss={loss:.4f}")
...
GATED_BEST_MODEL_DIR = str(RESULTS_DIR / "gated_best_model")
LAST_MODEL_DIR = str(RESULTS_DIR / "last_model")
CHECKPOINT_DIR = str(RESULTS_DIR / "finetune_checkpoints")
...
# Verify pretrain model exists
if not Path(PRETRAIN_MODEL_DIR).exists():
    logger.error(f"Pretrain model not found at {PRETRAIN_MODEL_DIR}. Run pretrain.py first.")
    sys.exit(1)
...
# ============================================================
# Data: real dataのみ（exp023と同一のsplit）
# ============================================================
...
bidirectional_train = create_bidirectional_data_from_df(train_split)
...
# ============================================================
# Training: pretrain modelからfine-tune
# ============================================================
gc.collect()
torch.cuda.empty_cache()
logger.info(f"Loading pretrain model from {PRETRAIN_MODEL_DIR}")
...
args = Seq2SeqTrainingArguments(
    ...
    logging_steps=10,
    report_to="wandb",
    run_name=f"s1_exp008_finetune_fold{FOLD}",
    load_best_model_at_end=False,
    seed=SEED,
    dataloader_num_workers=12,
)

metrics_logger = MetricsLogger(str(RESULTS_DIR / "finetune_metrics_log.json"))
gated_callback = GatedBestModelCallback(GATED_BEST_MODEL_DIR, tokenizer, loss_gate=1.2)
...
trainer = Seq2SeqTrainer(
    ...
    callbacks=[metrics_logger, gated_callback],
)
...
logger.info(f"Starting Stage2 Fine-tune (s1_exp008 fold{FOLD}, {EPOCHS} epochs on real data)...")
if resume_ckpt:
...
logger.info(f"Last model eval metrics: {metrics}")

import yaml
with open(str(RESULTS_DIR / "finetune_eval_metrics.yaml"), "w") as f:
    yaml.dump(metrics, f, default_flow_style=False)

logger.info(f"Stage2 Fine-tune complete. Fold {FOLD}")
logger.info(f"Gated best model: {GATED_BEST_MODEL_DIR}")
