"""
s1_exp008_large_st_pretrain: Stage1 — pseudo dataでpre-training
- exp034_st_pretrain/pretrain.pyベース
- 開始モデル: s1_exp007_large_lr1e4/fold3/last_model (byt5-large)
- pseudo_labels.csvを双方向学習データとして5epoch学習
- real dataのfold3 valでevalしてモニタリング
...
PSEUDO_LABELS_PATH = EXP_DIR / "results" / "pseudo_labels.csv"

# s1_exp007のlast_modelを開始モデルとして使用
BASE_MODEL_DIR = str(PROJECT_ROOT / "workspace" / "s1_exp007_large_lr1e4" / "results" / "fold3" / "last_model")
...
# ============================================================
# Data: pseudo dataで学習、real dataでeval
# ============================================================
# Pseudo data (学習用)
logger.info(f"Loading pseudo labels from {PSEUDO_LABELS_PATH}")
pseudo_df = pd.read_csv(str(PSEUDO_LABELS_PATH))
logger.info(f"Pseudo labels: {len(pseudo_df)} docs")
...
args = Seq2SeqTrainingArguments(
    ...
    logging_steps=10,
    report_to="wandb",
    run_name=f"s1_exp008_pretrain_fold{FOLD}",
...
logger.info(f"Starting Stage1 Pretrain (s1_exp008 fold{FOLD}, {EPOCHS} epochs on pseudo data)...")
if resume_ckpt:
