"""
s1_exp010_xl_lr5e5: byt5-xl版、lr=5e-5
- s1_exp007(byt5-large, lr=1e-4)ベース
- モデルをgoogle/byt5-xlに変更
- lr=5e-5（large比1/2）
- gradient_checkpointing有効化（VRAM対策）
- batch_size=2, grad_accum=4（effective batch=8、VRAM対策で調整）
"""
import os
import gc
import re
...
GATED_BEST_DIR = str(RESULTS_DIR / "gated_best_model")
...
args = Seq2SeqTrainingArguments(
    ...
    logging_steps=10,
    report_to="wandb",
    run_name=f"s1_exp010_xl_lr5e5_fold{FOLD}",
...
gated_callback = GatedBestModelCallback(GATED_BEST_DIR, tokenizer, loss_gate=1.2)
...
logger.info(f"Starting Training (s1_exp010 byt5-xl GKF fold{FOLD})...")
if resume_ckpt:
