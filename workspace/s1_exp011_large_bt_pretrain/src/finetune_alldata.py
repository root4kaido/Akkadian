"""
s1_exp011_large_bt_pretrain: Stage2 全データ版 — pretrain済みモデルでreal data全件をfine-tune
- Stage1のpretrain_model (byt5-large) をロード
- real data全件（fold分割なし）で5epoch fine-tune (lr=2e-5)
- validationなし、last modelのみ保存
"""
import os
import gc
import re
...
parser.add_argument("--pretrain-fold", type=int, default=3, help="Which fold's pretrain model to use")
cmd_args = parser.parse_args()
PRETRAIN_FOLD = cmd_args.pretrain_fold
...
RESULTS_DIR = EXP_DIR / "results" / "alldata"
RESULTS_DIR.mkdir(parents=True, exist_ok=True)

# Stage1のpretrain model
PRETRAIN_MODEL_DIR = str(EXP_DIR / "results" / f"fold{PRETRAIN_FOLD}" / "pretrain_model")

log_file = str(RESULTS_DIR / "finetune_alldata.log")
...
logger.info(f"Expanded Train Data (all): {len(train_expanded)} sentences")
...
bidirectional_train = create_bidirectional_data_from_df(train_expanded)
...
# ============================================================
# Training: pretrain modelからfine-tune（eval無し）
# ============================================================
gc.collect()
...
args = Seq2SeqTrainingArguments(
    ...
    logging_steps=10,
    report_to="wandb",
    run_name=f"s1_exp011_finetune_alldata",
    seed=SEED,
    dataloader_num_workers=12,
)

metrics_logger = MetricsLogger(str(RESULTS_DIR / "finetune_alldata_metrics_log.json"))
...
logger.info(f"Starting Stage2 Fine-tune alldata (s1_exp011, {EPOCHS} epochs on all real data)...")
if resume_ckpt:
...
logger.info(f"Stage2 Fine-tune alldata complete.")
